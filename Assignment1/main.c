...
unsigned int intToBinary(unsigned int k) {
	return (k == 0 || k == 1 ? k : ((k % 2) + 10 * intToBinary(k / 2)));
}
...
void PRVGADraw_Task(void *pvParameters)
{
	...
	alt_up_char_buffer_string(char_buf, "RoC Absolute (Hz/s)", 4, 26);
	...
	while (1)
	{
		...
		{ // i here points to the oldest data, j loops through all the data to be drawn on VGA
			...
			{
				alt_up_char_buffer_string(char_buf, "Normal     ", 14, 42);
			}
			else if (currentSystemState == 1)
			{
				alt_up_char_buffer_string(char_buf, "Load       ", 14, 42);
			}
			else if (currentSystemState == 2)
			{
				alt_up_char_buffer_string(char_buf, "Maintenance", 14, 42);
			}

			alt_up_char_buffer_string(char_buf, "Is Stable: ", 0, 44);
			alt_up_char_buffer_string(char_buf, itoa(readStabiliyVGA, systemStabilityBuffer, 10), 11, 44);

			alt_up_char_buffer_string(char_buf, "Freq Threshold (Hz): ", 0, 48);
			alt_up_char_buffer_string(char_buf, VGAFreqBuffer, 21, 48);
			alt_up_char_buffer_string(char_buf, "RoC Threshold (Hz/s): ", 0, 50);
			alt_up_char_buffer_string(char_buf, VGARocBuffer, 22, 50);
			...
			alt_up_char_buffer_string(char_buf, "Latest Times (ms): ", 60, 40);
...
{
	xTaskCreate(PRVGADraw_Task, "DrawTsk", configMINIMAL_STACK_SIZE, NULL, PRVGADraw_Task_P + 7, &PRVGADraw);
